<template>
  <div class="container mx-auto p-4">
    <h1 class="text-4xl font-bold mb-6 text-gray-800">Social Anti-Fake News System</h1>

    <div class="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
      <div class="flex space-x-2">
        <button
          @click="filterStatus = 'all'"
          :class="{'bg-blue-600 text-white': filterStatus === 'all'}"
          class="px-4 py-2 rounded-lg font-medium border transition-colors duration-200 hover:bg-blue-500 hover:text-white"
        >
          All
        </button>
        <button
          @click="filterStatus = 'not fake'"
          :class="{'bg-green-600 text-white': filterStatus === 'not fake'}"
          class="px-4 py-2 rounded-lg font-medium border transition-colors duration-200 hover:bg-green-500 hover:text-white"
        >
          Not Fake
        </button>
        <button
          @click="filterStatus = 'fake'"
          :class="{'bg-red-600 text-white': filterStatus === 'fake'}"
